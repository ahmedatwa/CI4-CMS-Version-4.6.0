...
class Twig
{
    ...
    */
    ...
    {        
        ...
        /**
         * Look for view file in extensions folder 
         * since file isn't located in default Views folder
        */
         if(! is_file($file) && str_starts_with($filename, 'Extensions')) {
            $file = $this->locator->locateFile($filename, 'Extensions/Views', empty($fileExt) ? 'twig' : $fileExt);
         }

         /**
         * since template file not found in default location
         * could be front end view then append the theme folder name
        ...
        try {
            ...
            $config = [
                ...
                'cache'            => config('paths')->writableDirectory . DIRECTORY_SEPARATOR . 'cache',
            ];
